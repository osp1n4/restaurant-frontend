...
function BarChart({ data, title, value, change }) {
  ...
  return (
    <div className="lg:col-span-2 flex flex-col gap-2 p-6 rounded-xl border border-slate-700 bg-slate-900 hover:shadow-xl transition-shadow">
        ...
        {data.map((item, index) => {
          ...
          return (
            <div key={index} className="contents">
              <div
                className="bg-primary/30 w-full rounded-t-md hover:bg-primary/40 transition-colors cursor-pointer"
                style={{ height: `${heightPercentage}%` }}
                title={`${item.name}: ${item.quantity} unidades`}
              />
              <p className="text-gray-400 text-[13px] font-bold leading-normal tracking-[0.015em] truncate max-w-full">
                {item.name}
              </p>
            </div>
          );
